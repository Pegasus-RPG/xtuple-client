...
/*
 *  Constructs a rptDetailedInventoryHistoryByLocation as a child of 'parent', with the
 *  name 'name' and widget flags set to 'f'.
 *
 *  The dialog will by default be modeless, unless you set 'modal' to
 *  true to construct a modal dialog.
 */
rptDetailedInventoryHistoryByLocation::rptDetailedInventoryHistoryByLocation(QWidget* parent, const char* name, bool modal, Qt::WFlags fl)
 ...
 */
rptDetailedInventoryHistoryByLocation::~rptDetailedInventoryHistoryByLocation()
{
 ...
 *  language.
 */
void rptDetailedInventoryHistoryByLocation::languageChange()
{
    retranslateUi(this);
}


void rptDetailedInventoryHistoryByLocation::init()
{
  _captive = FALSE;
...
enum SetResponse rptDetailedInventoryHistoryByLocation::set(ParameterList &pParams)
{
  _captive = TRUE;
  ...
  // calling sPopulateLocations here as the trigger for auto-update
  // is clicked() and the above changing does not cause it to be called.
  sPopulateLocations();
  ...
  param = pParams.value("transactionTypes", &valid);
  if (valid)
  {
...
void rptDetailedInventoryHistoryByLocation::sPopulateLocations()
{
  if (_warehouse->isAll())
  ...
  {
    XSqlQuery location;
    location.prepare( "SELECT location_id, "
                      ...
                      "WHERE (location_warehous_id=:warehous_id) "
                      "ORDER BY locationname;" );
    _warehouse->bindValue(location);
    location.exec();
    _location->populate(location);
  }
}

void rptDetailedInventoryHistoryByLocation::sPopulateLocationInfo(int pLocationid)
{
  XSqlQuery location;
  location.prepare( "SELECT formatBoolYN(location_netable) AS netable,"
                    "       formatBoolYN(location_restrict) AS restricted "
                    "FROM location, warehous "
                    "WHERE ( (location_warehous_id=warehous_id)"
                    " AND (location_id=:location_id) );" );
  location.bindValue(":location_id", pLocationid);
  location.exec();
  if (location.first())
  {
    _netable->setText(location.value("netable").toString());
    _restricted->setText(location.value("restricted").toString());
  }
}

void rptDetailedInventoryHistoryByLocation::sPrint()
{
  if (!_dates->allValid())
  {

...
void dspInvoiceInformation::sParseInvoiceNumber()
{
  q.prepare( "SELECT invchead_id, invchead_cust_id, invchead_ponumber,"
             ...
             "       formatMoney((invchead_misc_amount + invchead_freight + invchead_tax + SUM(COALESCE(round(invcitem_billed * invcitem_price / COALESCE(iteminvpricerat(item_id), 1),2),0)))) AS f_amount,"

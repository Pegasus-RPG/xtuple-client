...
void dspJournals::sViewSeries()
{
  ...
  dspViewSeries.prepare("SELECT sltrans_date, sltrans_journalnumber"
            "  FROM sltrans"
            " WHERE (sltrans_id=:sltrans_id)");
  dspViewSeries.bindValue(":sltrans_id", list()->id());
  dspViewSeries.exec();
  ...
  params.append("journalnumber", dspViewSeries.value("SLtrans_journalnumber").toString());
...
void dspJournals::sViewDocument()
{
  ...
  else if(item->rawValue("sltrans_doctype").toString() == "SO")
  {
    ...
    dspViewDocument.exec();
    if(dspViewDocument.first())
      salesOrder::viewSalesOrder(dspViewDocument.value("cohead_id").toInt(), this);
  }
  else if(item->rawValue("sltrans_doctype").toString() == "WO")
  {
    QStringList docnumber = item->rawValue("docnumber").toString().split("-");
  ...
  {
    dspViewDocument.prepare("SELECT sltrans_misc_id"
              "  FROM sltrans"
              " WHERE (sltrans_id=:sltrans_id)");
    dspViewDocument.bindValue(":sltrans_id", item->id());
    dspViewDocument.exec();
    if(!dspViewDocument.first())
      return;

    params.append("mode", "view");
    params.append("invhist_id", dspViewDocument.value("sltrans_misc_id").toInt());

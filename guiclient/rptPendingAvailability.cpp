...
/*
 *  Constructs a rptPendingAvailability as a child of 'parent', with the
 *  name 'name' and widget flags set to 'f'.
 *
 *  The dialog will by default be modeless, unless you set 'modal' to
 *  true to construct a modal dialog.
 */
rptPendingAvailability::rptPendingAvailability(QWidget* parent, const char* name, bool modal, Qt::WFlags fl)
 ...
 */
rptPendingAvailability::~rptPendingAvailability()
{
 ...
 *  language.
 */
void rptPendingAvailability::languageChange()
{
    retranslateUi(this);
}


void rptPendingAvailability::init()
{
  _captive = FALSE;
  ...
  _buildDate->setNullString(tr("Now"));
...
enum SetResponse rptPendingAvailability::set(ParameterList &pParams)
{
  _captive = TRUE;
  ...
  param = pParams.value("buildDate", &valid);
  if (valid)
    _buildDate->setDate(param.toDate());

  param = pParams.value("effectiveDate", &valid);
  if (valid)
    _effective->setDate(param.toDate());

  param = pParams.value("qtyToBuild", &valid);
  if (valid)
    _qtyToBuild->setText(formatQty(param.toDouble()));
...
void rptPendingAvailability::sPrint()
{

...
SetResponse taxBreakdown::set(const ParameterList& pParams)
{
  ...
  QString sql =
	...
	"         SUM(ROUND((invcitem_billed * invcitem_price) / "
	"                  (CASE WHEN(item_id IS NULL) THEN 1 "
	"			ELSE iteminvpricerat(item_id) END), 2)) AS line,"
	...
	"         SUM(ROUND((cmitem_qtycredit * cmitem_unitprice) / "
	"                  (CASE WHEN(item_id IS NULL) THEN 1 "
	"			ELSE iteminvpricerat(item_id) END), 2)) AS line,"
	...
	"         SUM(ROUND((coitem_qtyord * coitem_price) / iteminvpricerat(item_id),"
	"                    2)) AS line,"
	"         SUM(ROUND((calculateTax(coitem_tax_id,"
	"                        (ROUND((coitem_qtyord * coitem_price) /"
	"                        iteminvpricerat(item_id), 2)), 0, 'A')), 2)) AS linea,"
	"         SUM(ROUND((calculateTax(coitem_tax_id,"
	"                        (ROUND((coitem_qtyord * coitem_price) /"
	"                        iteminvpricerat(item_id), 2)), 0, 'B')), 2)) AS lineb,"
	"         SUM(ROUND((calculateTax(coitem_tax_id,"
	"                        (ROUND((coitem_qtyord * coitem_price) /"
	"                        iteminvpricerat(item_id), 2)), 0, 'C')), 2)) AS linec,"
	...
	"         SUM(ROUND((quitem_qtyord * quitem_price) / iteminvpricerat(item_id),"
	"                    2)) AS line,"
	"         SUM(ROUND((calculateTax(quitem_tax_id,"
	"                        (ROUND((quitem_qtyord * quitem_price) /"
	"                        iteminvpricerat(item_id), 2)), 0, 'A')), 2)) AS linea,"
	"         SUM(ROUND((calculateTax(quitem_tax_id,"
	"                        (ROUND((quitem_qtyord * quitem_price) /"
	"                        iteminvpricerat(item_id), 2)), 0, 'B')), 2)) AS lineb,"

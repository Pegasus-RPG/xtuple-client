...
SetResponse taxBreakdown::set(const ParameterList& pParams)
{
  ...
  QString sql =
	...
	"			ELSE item_invpricerat END), 2)) AS line,"
	...
	"         SUM(ROUND((coitem_qtyord * coitem_price) / item_invpricerat,"
	"                    2)) AS line,"
	"         SUM(ROUND((calculateTax(coitem_tax_id,"
	"                        (ROUND((coitem_qtyord * coitem_price) /"
	"                        item_invpricerat, 2)), 0, 'A')), 2)) AS linea,"
	"         SUM(ROUND((calculateTax(coitem_tax_id,"
	"                        (ROUND((coitem_qtyord * coitem_price) /"
	"                        item_invpricerat, 2)), 0, 'B')), 2)) AS lineb,"
	"         SUM(ROUND((calculateTax(coitem_tax_id,"
	"                        (ROUND((coitem_qtyord * coitem_price) /"
	"                        item_invpricerat, 2)), 0, 'C')), 2)) AS linec,"
	...
	"         SUM(ROUND((quitem_qtyord * quitem_price) / item_invpricerat,"
	"                    2)) AS line,"

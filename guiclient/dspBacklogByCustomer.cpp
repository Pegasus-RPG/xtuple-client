...
void dspBacklogByCustomer::sFillList()
{
  ...
  QString sql( "SELECT cohead_id, coitem_id, cohead_number, coitem_linenumber,"
               ...
               "       formatMoney(round(noNeg(coitem_qtyord - coitem_qtyshipped + coitem_qtyreturned) * (coitem_price / iteminvpricerat(item_id)),2)) AS f_backlog,"
               "       round(noNeg(coitem_qtyord - coitem_qtyshipped + coitem_qtyreturned) * (coitem_price / iteminvpricerat(item_id)),2) AS backlog "

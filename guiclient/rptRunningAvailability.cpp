...
rptRunningAvailability::rptRunningAvailability(QWidget* parent, const char* name, bool modal, Qt::WFlags fl)
...
rptRunningAvailability::~rptRunningAvailability()
{
    // no need to delete child widgets, Qt does it all for us
}

void rptRunningAvailability::languageChange()
{
    retranslateUi(this);
}

enum SetResponse rptRunningAvailability::set(const ParameterList &pParams)
{
  _captive = TRUE;
  ...
  param = pParams.value("showPlanned", &valid); 
  if (valid)
    _showPlanned->setChecked(TRUE);
...
void rptRunningAvailability::sPrint()
{
    if(!_item->isValid()) {
...
void rptRunningAvailability::sPopulateItemInfo()
{
  if (_item->isValid())
  {
    q.prepare( "SELECT itemsite_id, itemsite_qtyonhand,"
    ...
    if (q.first())
    {
      _qoh->setText(q.value("qoh").toString());
      _reorderLevel->setText(q.value("reorderlevel").toString());
      _orderMultiple->setText(q.value("multorderqty").toString());
      _orderToQty->setText(q.value("ordertoqty").toString());
    }
  }
  else
  {

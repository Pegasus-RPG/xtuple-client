...
/*
 *  Constructs a scrapTrans as a child of 'parent', with the
...
enum SetResponse scrapTrans::set(ParameterList &pParams)
{
  QVariant param;
  bool     valid;
  int      invhistid = -1;
  ...
  if (valid)
  {
    ...
    else if (param.toString() == "view")
    {
      ...
      q.prepare( "SELECT invhist_itemsite_id, invhist_transdate,"
                 ...
                 "       invhist_transtype, invhist_ordnumber,"
                 "       invhist_user "
                 "FROM invhist "
...
void scrapTrans::sPost()
{
  ...
  {
    QMessageBox::warning(  this, tr("Enter Scrap Quantitiy"),
                           tr(  "You must enter a valid Scrap Quantity before entering this\n"
                                "Scrap Transaction."  ));
    _qty->setFocus();
    return;
  }

...
void apCreditMemoApplication::populate()
{
  q.prepare( "SELECT apopen_vend_id, apopen_docnumber, apopen_doctype,"
	     ...
	     "       apopen_curr_id "
             ...
             "                               SUM(currToCurr(apchkitem_curr_id, apopen_curr_id, apchkitem_amount + apchkitem_discount, apchkitem_docdate)) AS prepared"
             "                          FROM apchk JOIN apchkitem ON (apchkitem_apchk_id=apchk_id)"
             "                                     JOIN apopen ON (apchkitem_apopen_id=apopen_id)"

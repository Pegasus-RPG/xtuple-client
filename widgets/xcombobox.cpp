...
static QString termsMQL("SELECT terms_id,"
                        ...
                        " ORDER by terms_code;");
static QString usrMQL("SELECT DISTINCT usesysid::INTEGER, usename::TEXT, usename::TEXT"
                      "  FROM pg_user"
                      "  LEFT OUTER JOIN usrpref ON usrpref_username=usename"
                      " WHERE true"
                      " <? if exists('activeOnly') ?> AND (usrpref_name='active'"
                      "                                    AND usrpref_value='t') <? endif ?>"
                      " <? if exists('isAgent') ?>    AND (usrpref_name='agent'"
                      "                                    AND usrpref_value='t') <? endif ?>"
                      " ORDER BY usename;");

...
{
  setInvoiceNumber(text().toInt());
  _id = _invoiceNumber;
}

void InvoiceLineEdit::setInvoiceNumber(int pInvoiceNumber)
{
  XSqlQuery query;
  query.prepare( "SELECT cohead_id, invchead_cust_id "
  ...
  if (query.first())
  {
    ...
    setText(QString("%1").arg(_invoiceNumber));
  }
  else
  {
    _custid = -1;
